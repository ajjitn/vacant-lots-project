...
lots = readRDS("final_data/lots_noper1000.rds")
...
make_eda_table = function(lots, radius){
    ...
    if (test == "t"){
      r = cols %>%
        ...
        mutate(crimetype = stringr::str_replace_all(stat, "std_diff_", ""),
               crimetype = stringr::str_replace_all(crimetype, paste0("_",radius_m, "_per_1000m"), ""),
  ...
  s = lots_radius %>% 
    ...
    summarize(
      ...
      abs_diff_g_per_1000 =      dplyr::nth(greened, n =2),
      std_abs_diff_g_per_1000 = dplyr::nth(greened, n = 3),
      abs_diff_c =       dplyr::nth(ungreened, n = 1),
      abs_diff_c_per_1000 =      dplyr::nth(ungreened, n =2),
      std_abs_diff_c_per_1000 = dplyr::nth(ungreened, n = 3)) %>% 
    ungroup() %>%
    mutate(diff_abs_diff_per_1000 = abs_diff_g_per_1000 - abs_diff_c_per_1000,
           diff_std_diff_per_1000 = std_abs_diff_g_per_1000 - std_abs_diff_c_per_1000) %>%
...
write_csv(lots_70m_eda, "final_data/lots_70m_eda_disjoint_notper1000.csv")
write_csv(lots_150m_eda, "final_data/lots_150m_eda_disjoint_notper1000.csv")
